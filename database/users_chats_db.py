...
class Database:
    ...
    async def add_join_request(self, user_id, chat_id):
        ...
        await self.join_requests.insert_one(join_request_data)

    async def check_join_request(self, user_id, chat_id):
        # Check if the user has a join request in the database
        result = await self.join_requests.find_one({"user_id": user_id, "chat_id": chat_id}) 
        if result:
    ...
    async def get_link(self, key, default):
        data = await self.link.find_one({"_id": key})
        if data:
            return data["value"]
        else:
            await self.add_link(key, default )
            return default 
            
    async def add_link(self, key, value):
        await self.link.insert_one({"_id": key, "value": value})

    
    async def update_link(self, data):
        await self.link.update_one({}, {"$set": data}, upsert=True)
        
    async def delete_all_link(self):
        await self.link.delete_many({})
